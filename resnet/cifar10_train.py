...
class Train(object):
    ...
    def placeholders(self):
        ...
        '''
        在 Tensorflow 中
        需要定義 placeholder 的 type 像是宣告變數空間，shape 就像是維度，主要用於input
        而 Variable 可以當成全域變數，須設定初始值，通常用於Weight, bias...etc
        '''
    ...
    def build_train_validation_graph(self):
        ...
        '''
        '''
        Variable 中的 trainable 表示 Optimizer 可不可更新
        '''
    ...
    def train(self):
        ...
        for step in range(FLAGS.train_steps):
            ...
            #train_batch_data, train_batch_labels = all_data, all_labels

            #validation_batch_data, validation_batch_labels = vali_data, vali_labels

            validation_batch_data, validation_batch_labels = self.generate_vali_batch(vali_data,
                                                           vali_labels, FLAGS.validation_batch_size)

            #if step < FLAGS.blur_steps:
            #    train_batch_data = tf.image.resize(tf.image.resize(train_batch_data,(8, 8)), (32, 32))
